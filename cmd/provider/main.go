...
import (
	...
	apisCluster "github.com/valkiriaaquatica/provider-proxmox-bpg/apis/cluster"
	apisNamespaced "github.com/valkiriaaquatica/provider-proxmox-bpg/apis/namespaced"
	"github.com/valkiriaaquatica/provider-proxmox-bpg/config"
	"github.com/valkiriaaquatica/provider-proxmox-bpg/internal/clients"
	controllerCluster "github.com/valkiriaaquatica/provider-proxmox-bpg/internal/controller/cluster"
	controllerNamespaced "github.com/valkiriaaquatica/provider-proxmox-bpg/internal/controller/namespaced"
	"github.com/valkiriaaquatica/provider-proxmox-bpg/internal/features"
	"github.com/valkiriaaquatica/provider-proxmox-bpg/internal/version"
)
...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for ProxmoxBPG").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-proxmox-bpg"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-proxmox-bpg",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped Proxmox APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced Proxmox APIs to scheme")
	...
	if *enableChangeLogs {
		...
		clo := xpcontroller.ChangeLogOptions{
			ChangeLogger: managed.NewGRPCChangeLogger(
				...
				managed.WithProviderVersion(fmt.Sprintf("provider-proxmox-bpg:%s", version.Version))),
		}
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped ProxmoxBPG controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced ProxmoxBPG controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped ProxmoxBPG controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced ProxmoxBPG controllers")
	}
