...
export TERRAFORM_PROVIDER_VERSION ?= 0.106.0
...
export TERRAFORM_NATIVE_PROVIDER_BINARY ?= terraform-provider-proxmox_v0.106.0
...
xpkg.build.provider-proxmox-bpg: do.build.images
...
pull-docs:
	@if [ ! -d "$(WORK_DIR)/$(TERRAFORM_PROVIDER_SOURCE)" ]; then \
		...
		git clone -c advice.detachedHead=false --depth 1 --filter=blob:none --branch main --sparse "$(TERRAFORM_PROVIDER_REPO)" "$(WORK_DIR)/$(TERRAFORM_PROVIDER_SOURCE)"; \
	fi
